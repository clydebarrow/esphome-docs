...
for more information. The preferences are stored in a file in a directory specified by the environment
variable `ESPHOME_PREFDIR` or if that is unset, in a directory named `.esphome/prefs` under the user's home directory.
If neither `ESPHOME_PREFDIR` is set nor the home directory can be determined from the `HOME` environment variable,
preferences will not be saved or restored, and error messages will be logged.
