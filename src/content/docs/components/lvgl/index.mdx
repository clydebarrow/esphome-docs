...
The display orientation may be changed at runtime with the `lvgl.set_rotation` action, which accepts the same values as the configuration option.
When using this action, the touchscreen will be automatically adjusted to match the new orientation.
...
This [action](/automations/actions#actions-action) changes the display rotation at runtime. The touchscreen will be automatically adjusted to match the new orientation.
- **rotation** (**Required**): The rotation to set. Valid values are `0`, `90`, `180` and `270`.
