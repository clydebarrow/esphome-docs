...
- [interaction](#triggers) LVGL event triggers which also return the value in `x`. These apply to the widget itself, not to the sliders inside it, so they are sent only when its own background is touched.
...
> [!NOTE]
> The `on_value` trigger is sent continuously while a slider is being dragged, so keep the automation light. If it does anything expensive, put the work in a [script](/components/script/) with `mode: restart` and a short delay, so that only the last color in a drag is acted on.
