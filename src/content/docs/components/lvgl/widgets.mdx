...
The color picker lets the user choose a color by dragging sliders. It is built from six sliders: hue, saturation and brightness, and one for each of the red, green and blue channels. Moving any slider updates all the others, along with a swatch of the chosen color and its hex value. Each slider's knob is filled with the color its own bar shows at the current position.
...
- **items** (*Optional*, list): Settings for the bars of all six sliders. Supports a list of [styles](/components/lvgl#lvgl-styling) and state-based styles to customize. Each slider already has a color gradient of its own, so setting a background color here will hide it.
- **knob** (*Optional*, list): Settings for the knob *part* of all six sliders. Supports a list of [styles](/components/lvgl#lvgl-styling) and state-based styles to customize. See the note below about `bg_color`.
