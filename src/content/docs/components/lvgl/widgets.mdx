...
- **width** (**Required**): Width of the widget in pixels or a percentage, or `SIZE_CONTENT`. It is square, so the height is always set to match, and giving a `height` is an error. With `SIZE_CONTENT` the widget sizes itself from the height of a line of its `text_font`, so setting a larger font makes the whole widget larger.
