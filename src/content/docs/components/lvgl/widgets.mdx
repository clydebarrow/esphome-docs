...
- **id** (*Optional*): The ID of a widget configured in LVGL to refresh, or a list of widgets.

Only properties with lambdas are refreshed. A build-time error will be raised if a widget is refreshed but it does not have any properties with lambdas.
If no widgets are specified, then all widgets with lambda properties will be refreshed.
