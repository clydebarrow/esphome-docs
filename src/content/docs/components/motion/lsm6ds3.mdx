...
As an IMU with both an accelerometer and a gyroscope, the LSM6DS3 supports all of the
[Motion](/components/motion) sensor types, as well as the orientation
[binary sensors](/components/motion#binary-sensor) (`face_up`, `face_down`, `free_fall`, `moving`)
and the shake [event](/components/motion#event).
