...
- **data_rate** (*Optional*): The SPI data rate. Defaults to 10MHz but board presets may override this.
  ...
  and test higher rates to find what works. A MISO pin should preferably not be specified, as this may limit the maximum rate in some circumstances,
...
- **transform** (*Optional*): Can be used to mirror and/or swap the x and y axes. Should not be required unless using the `CUSTOM` model.
  ...
  If this option is specified, then the `dimensions` option must also be provided. The value can either be the string `disabled` to disable hardware transform, or a dictionary.
