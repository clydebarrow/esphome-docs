...
| UC8179                 | UltraChip    | E-paper controller used by 800x480 7.5" panels (mono and 3-color variants) |
...
| Waveshare-7.5in-V2        | Waveshare           | [https://www.waveshare.com/7.5inch-e-paper-hat.htm](https://www.waveshare.com/7.5inch-e-paper-hat.htm) — 7.5" V2 mono (800x480, UC8179), supports fast and partial refresh via `full_update_every` |
...
| Seeed-reTerminal-Sticky | Seeed Studio | 800x480 e-paper (SSD1677), mono mode, supports partial refresh. [https://www.seeedstudio.com/sticky/](https://www.seeedstudio.com/sticky/) |
| Seeed-reTerminal-Sticky-Gray4 | Seeed Studio | 800x480 e-paper (SSD1677), 4 level grayscale mode, full refresh only. [https://www.seeedstudio.com/sticky/](https://www.seeedstudio.com/sticky/) |
...
- **busy_pin** (*Optional*, [Pin Schema](/guides/configuration-types#pin-schema)): The BUSY pin, if used.
  ...
  > on the busy pin. This applies to: `Waveshare-1.54in-G`, `Waveshare-7.5in-H`, `Waveshare-7.5in-V2`,
  > `inkplate6color`.
...
- **full_update_every** (*Optional*, int): On screens that support partial updates, this sets the number of updates
  before a full update is forced. Defaults to `1` which will make every update a full update. Models that don't
  support partial update (e.g. `Seeed-reTerminal-Sticky-Gray4`) reject any value other than `1`.
- **border_waveform** (*Optional*, int): SSD1677-based models only (`SSD1677`, `Waveshare-4.26in`, `Waveshare-3.97in`,
  `Seeed-ee04-mono-4.26`, `Seeed-reTerminal-Sticky`, `Seeed-reTerminal-Sticky-Gray4`). Sets the panel's border
  waveform register value, e.g. `0x01`. Defaults to `0x01` (`0x00` for `Seeed-reTerminal-Sticky-Gray4`).
...
### `epaper_spi.full_update_next` Action

Forces the next display update to be a full refresh instead of a partial one. Useful with
`full_update_every` when a full refresh is needed sooner than the configured interval, for
example right after a large content change.

```yaml
on_...:
  then:
    - epaper_spi.full_update_next: my_display
```
...
### Fast and partial refresh example (reTerminal E1001)

Displays based on the UC8179 controller (`Waveshare-7.5in-V2`, `Seeed-reTerminal-E1001`) support fast full and
partial refresh. Set `full_update_every` greater than `1` to enable it: a fast full refresh is performed every
`full_update_every` updates, with partial refreshes in between. With the default of `1`, every update is a normal
full refresh.
