...
To trigger a roaming check immediately instead of waiting for the periodic check, use the
[`wifi.roam` action](#wifi-roam).
...
<span id="wifi-roam"></span>

### `wifi.roam` Action

This action asks the WiFi component to check right away for a better access point on the current network, as
described in [Post-Connect Roaming](#post-connect-roaming). It skips the usual 5-minute wait between checks, but
still only roams if a meaningfully stronger access point is actually found.
...
The action is ignored if the device is not currently connected, or if a roaming check is already in progress.
