...
- **acceleration_range** (*Optional*, string): The full-scale range of the accelerometer. One of
  `2G`, `4G`, `8G`, `16G`. Defaults to `4G`.
- **acceleration_odr** (*Optional*, string): The output data rate of the accelerometer. One of
...
- **update_interval** (*Optional*, [Time](/guides/configuration-types#time)): The interval at which
  sensor data is read. Defaults to `50ms`.
...
- **type** (**Required**, string): The type of measurement to expose. One of:
  ...
  - `gyroscope_x`: X-axis angular velocity in `°/s`.
  - `gyroscope_y`: Y-axis angular velocity in `°/s`.
  - `gyroscope_z`: Z-axis angular velocity in `°/s`.
