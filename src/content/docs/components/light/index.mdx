...
- **color** (*Optional*, string): A
  [standard CSS color name](https://developer.mozilla.org/en-US/docs/Web/CSS/named-color),
  for example `red` or `springgreen`, used instead of **red**, **green** and **blue**. Names are not case-sensitive.
  Cannot be used together with **red**, **green** or **blue**. This option is not templatable.

  The light always drives its brightest color channel at full level. For a dark color such as `darkred`, the
  **color_brightness** is therefore set to match (about 55% here). Colors with a channel at full level, such as `red`,
...
- **color** (*Optional*, string): A
  [standard CSS color name](https://developer.mozilla.org/en-US/docs/Web/CSS/named-color)
  to use instead of **red**, **green** and **blue**. Not templatable. Also sets **color_brightness**, as
  described in [light state](#light-state). Unlike **red**, **green** and **blue**, which use the light's current
  **color_brightness** when it is not given, a color name always sets **color_brightness** for the range.
