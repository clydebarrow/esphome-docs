...
> [!NOTE]
> When a light is OFF, reading its current `brightness` will report `0.0`. The light's effective stored brightness will
> be treated as `100%` for the next turn-on if no `brightness` is explicitly provided. To ensure a specific brightness on
> turn-on, include `brightness` in the `turn_on` call.
