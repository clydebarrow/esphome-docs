...
- **initial_state** (*Optional*): The initial state the light should be set to on bootup, applied before any state
  ...
  **initial_state** for the fields they touch.
  ...
  > [!WARNING]
  > Before 2026.10.0, a light whose `initial_state` set `state: ON` and had neither `restore_mode` nor `restore_state`
  ...
  If **color_mode** is not given but color fields such as `red` or `brightness` are, the light uses the first
  color mode it supports that provides those fields (for example `RGB` for `red`, `green` and `blue`).
...
- **restore_state** (*Optional*): Persist the light's state to flash and restore it on bootup, with optional
  ...
  - **state** (*Optional*, string or boolean): `KEEP` (default, case-insensitive) to restore the saved ON/OFF
    state as-is, `INVERT` to restore it inverted, `INITIAL` to use **initial_state**'s state instead (OFF if
    **initial_state** doesn't set one), or an explicit `ON`/`OFF` (or boolean) to always force this value
...
- **restore_mode** (*Optional*): Control how the light attempts to restore state on bootup. Mutually exclusive
  ...
  To migrate from `restore_mode` to `restore_state`, replace it with the equivalent configuration below. For the
  `*_OFF` values this assumes **initial_state** does not set `state: ON`; remove that if it does, since
  `restore_state` and "set neither" honor **initial_state** where `restore_mode` overrode it.
  ...
  `ALWAYS_OFF`: set neither `restore_mode` nor `restore_state`.

  `ALWAYS_ON`: set neither `restore_mode` nor `restore_state`, and:
...
- **color** (*Optional*, string or hex value): A
  [standard CSS color name](https://developer.mozilla.org/en-US/docs/Web/CSS/named-color),
  for example `red` or `springgreen`, or a hex value in the form `0xRRGGBB`, for example `0x223344`, used instead of
  **red**, **green** and **blue**. Names are not case-sensitive.
  Cannot be used together with **red**, **green** or **blue**. This option is not templatable.

  The light always drives its brightest color channel at full level. For a dark color such as `darkred` or
  `0x223344`, the **color_brightness** is therefore set to match (about 55% and 27% respectively). Colors with a channel at full level, such as `red`,
  set **color_brightness** to 100%. If you also set **color_brightness**, your value is used instead, and a warning
  is logged if the color is a dark one.

  Color names such as `white` and `gray` are converted to RGB colors only and do not affect the white channel,
  if present.

- **white** (*Optional*, percentage, [templatable](/automations/templates)): The brightness of the white channel.
- **color_temperature** (*Optional*, float, [templatable](/automations/templates)): The color temperature
  (in [mireds](https://en.wikipedia.org/wiki/Mired) or Kelvin) of the white channel.
...
- **color** (*Optional*, string or hex value): A
  [standard CSS color name](https://developer.mozilla.org/en-US/docs/Web/CSS/named-color)
  or a hex value in the form `0xRRGGBB` to use instead of **red**, **green** and **blue**. Not templatable. Also sets **color_brightness**, as
  described in [light state](#light-state). Unlike **red**, **green** and **blue**, which use the light's current
  **color_brightness** when it is not given, a `color` always sets **color_brightness** for the range.
